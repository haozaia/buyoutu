  ...
  <div id="fiveTwoDialog">
    ...
    >
      ...
      <el-row>
        ...
        <el-col span="8">
          <el-form-item prop="existingScale" label="现有规模：">
      ...
      <el-row>
        附件列表：
          <span class="curPoi " style="margin-right:50px;" v-for='(item , index) in downloadList' :key="index" @click="downloadFile(item.id)">{{item.projectName}}</span>
      </el-row>
    </el-form>
  </div>
</template>
...
export default {
  ...
  data() {
    ...
    return {
      ...
      rules: {
        collectCompany:  [{ required: true, message: '请输入项目名称', trigger: 'blur' }],
        projectName:  [{ required: true, message: '请选择项目来源', trigger: 'blur' }],
        projectSource:  [{ required: true, message: '请选择往来单位', trigger: 'blur' }],
      ...
      this.axios({
        ...
        var datas= res.data.data;
         datas.belongToIndustry = JSON.parse(datas.belongToIndustry);
          ...
          if(this.$store.state.unitCode == 2){
            datas.projectSource = '******'
            datas.isEnvironmental = '******'
            datas.targetMarket = '******'
          }
      });
    },
